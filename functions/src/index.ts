...
const onScheduler = functions.pubsub
  ...
  .onRun(async () => {
    ...
    try {
      let idList: string[] = [];
      const collectionRef = collection(db, "userPlatform");
      const doxSnap = await getDocs(collectionRef);
      doxSnap.forEach((doc) => {
        idList.push(doc.id);
      });
      if (idList.length > 0) {
 ...
 */
async function initCleaner(accounts: string[]) {
 ...
 */
async function deleteStorage(accountID: string, eventID: string, name: string) {
  try {
      ...
      .then(async (event) => {
        ...
        await deleteDoc(doc(db, path))
          .then(async (e) => {
            ...
            const servicesRef = doc(db, pathServ);
            const model = {
              name: name,
              id: eventID,
            };
            await updateDoc(servicesRef, {
              services: arrayRemove(model),
            }).then(() => {
...
const onSchedulerCurrency = functions.pubsub
  ...
  .onRun(async () => {
    ...
    /** Interval to Clean database */ try {
      let path =
        "http://api.exchangeratesapi.io/v1/latest?access_key=0b944b44a771e12fb004a5098b82a600&symbols=USD,MXN,BRL,CLP,COP,BOB,ARS,PEN,CUP";

      request(path, (err: any, response: any, body: any) => {
...
async function saveCurrency(value: any) {
  try {
...
export const onDeleteHandler = onDelete;
export const onSchedulerHandler = onScheduler;
export const onSchedulerAlarmHandler = onSchedulerAlarms;
export const onSchedulerCurrencyHandler = onSchedulerCurrency;
